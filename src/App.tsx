...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const handleTaskVerify = async (task: any) => {
    ...
    try {
      ...
      // Check if this was a "Must Task" for referral counting
      if (task.isMustTask && userData?.referredBy && !userData.referralCounted) {
        // Find referrer
        const referrerQuery = query(collection(db, 'users'), where('telegramId', '==', userData.referredBy));
        ...
        if (!referrerSnapshot.empty) {
          ...
          // Mark as counted
          await updateDoc(userRef, { referralCounted: true });
        }
      }
