...
export default function App() {
  ...
  const defaultSettings: AppSettings = {
    ...
    appLogo: 'https://picsum.photos/seed/money/200',
    earningPerAd: 0.0004,
...
━━━━━━━━━━━━━━━━━━━━━`,
  ...
  const handleCopyReferLink = () => {
    ...
    const link = `https://t.me/${currentSettings.botUsername}?start=${userData.telegramId}`;
  ...
  const handleTaskComplete = async (task: any) => {
    if (!user || !currentSettings || verifyingTask || cooldowns[task.id] || userData?.tasksCompleted.includes(task.id)) return;

    if (task.type === 'TELEGRAM_CHANNEL') {
      if (!tgUser) {
        setError('Please open in Telegram to complete this task.');
        return;
      }

      setVerifyingTask(task.id);
      try {
        const response = await fetch('/api/telegram/check-member', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            botToken: currentSettings.botToken,
        ...
        if (data.isMember) {
          ...
          setCooldowns(prev => ({ ...prev, [task.id]: 10 }));
        } else {
          setVerifyingTask(null);
...
━━━━━━━━━━━━━━━━━━━━━`;
      ...
      if (currentSettings.botToken && currentSettings.withdrawChannel) {
        await fetch(`https://api.telegram.org/bot${currentSettings.botToken}/sendMessage`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            chat_id: currentSettings.withdrawChannel,
            text: message,
          })
        });
      }
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-center overflow-hidden">
                          <img 
                            src={task.logoUrl || 'https://cdn-icons-png.flaticon.com/512/2111/2111646.png'} 
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">+${task.reward.toFixed(2)}</span>
                      ...
                      <button 
                        ...
                        disabled={verifyingTask === task.id || cooldowns[task.id] > 0}
                        className={cn(
                      ...
                      >
                        {verifyingTask === task.id ? (
                          ...
                          <><Timer className="w-3 h-3" /> {cooldowns[task.id]}s</>
                        ) : (
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-3xl flex items-center gap-3">
                  <input 
                    ...
                    value={userData?.telegramId && userData.telegramId !== 'unknown' 
                      ? `https://t.me/${currentSettings.botUsername}?start=${userData.telegramId}`
              ...
              <div className="p-6 space-y-3">
                {currentSettings.supportLinks && currentSettings.supportLinks.length > 0 ? (
                  currentSettings.supportLinks.map((link) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-purple-500/10 rounded-xl group-hover:bg-purple-500 group-hover:text-white transition-all">
                          {link.platform === 'telegram' && <Smartphone className="w-4 h-4" />}
                          {link.platform === 'facebook' && <Users className="w-4 h-4" />}
                          {link.platform === 'youtube' && <PlayCircle className="w-4 h-4" />}
                          {link.platform === 'instagram' && <UserIcon className="w-4 h-4" />}
                          {link.platform === 'tiktok' && <Smartphone className="w-4 h-4" />}
                        </div>
